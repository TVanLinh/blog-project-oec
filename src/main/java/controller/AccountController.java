...
public class AccountController {
    ...
    public  String userInfor(Principal principal,HttpServletRequest request,ModelMap modelMap,@RequestParam(value = "page",required = false) String pageRequest) {
        ...
        postList = this.postService.finAll(this.portSort.getQuerySortAllPostByUserName(request,(page-1)* NumberViewSort.NUMBER_VIEW,user));
        this.requestService.setPostList(modelMap,postList,this.postService.getPostByIdUser(user.getId()).size(),page);
        return "author";
    }
    ...
    public  String searchTableAllPost(HttpServletRequest request,ModelMap modelMap,@RequestParam(value = "page",required = false)String pageRequest,@RequestParam(value = "query_search",required = false)String querySearch, Principal principal) {
        ...
        int page = NumberUtils.toInt(pageRequest,1);
        this.requestService.setPostList(modelMap,this.postService.getPostByIdUser(sortType,querySearch,user.getId(),(page-1)*NumberViewSort.getNumberView()),this.postService.getCountByUserContainsTitle(querySearch,user.getId()),page,null,null,querySearch);
